...
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        cBaseDatos2 objDB = new cBaseDatos2();
        
 
        String estado = objDB.validarUsuario(xnom, xcla);
        
if (estado != null) {
    HttpSession misession = request.getSession(true);
    misession.setAttribute("usuario", xnom.toUpperCase());
    misession.setAttribute("estado", estado.toUpperCase());
    response.sendRedirect(request.getContextPath() + "/entorno.jsp"); // Redirige a entorno.html
} else {
